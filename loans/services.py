...
class InterestCalculationService:
    """
    Professional interest calculation service following banking standards.
    Supports multiple interest calculation methods and day count conventions.
    """
    
    # Day count conventions
    DAY_COUNT_METHODS = {
        'ACTUAL_365': {'days_in_year': 365, 'use_actual_days': True},
        'ACTUAL_360': {'days_in_year': 360, 'use_actual_days': True},
        '30_360': {'days_in_year': 360, 'use_actual_days': False},
    }
    
    @classmethod
    def calculate_days_between(cls, start_date: datetime.date, end_date: datetime.date, 
                              method: str = 'ACTUAL_365') -> int:
        """
        ...
        Returns:
            Number of days according to convention
        """
        if method == '30_360':
    ...
    @classmethod
    def calculate_flat_interest(cls, principal: Decimal, annual_rate: Decimal, 
                               days: int, method: str = 'ACTUAL_365') -> Decimal:
        """
        Calculate flat (simple) interest.
        
        Formula: I = P × r × (days/days_in_year)
        
        Args:
            principal: Loan principal
            annual_rate: Annual interest rate (as percentage, e.g., 15 for 15%)
            days: Number of days
            method: Day count convention
        ...
        # Convert annual rate to decimal (15% -> 0.15)
    ...
    @classmethod
    def calculate_reducing_balance_interest(cls, principal: Decimal, annual_rate: Decimal,
                                          start_date: datetime.date, end_date: datetime.date,
                                          repayment_dates: List[datetime.date],
                                          repayment_amounts: List[Decimal],
                                          method: str = 'ACTUAL_365') -> Decimal:
        """
        Calculate reducing balance interest with actual repayment dates.
        ...
        Args:
            ...
            repayment_dates: List of repayment dates
            repayment_amounts: List of principal repayments
            method: Day count convention
        ...
        # Calculate interest for final period if any
    ...
    @classmethod
    def calculate_amortized_payment(cls, principal: Decimal, annual_rate: Decimal,
                                   term_months: int, payment_frequency: str = 'MONTHLY') -> Decimal:
        """
        Calculate fixed payment for amortized loan (equal installments).
    ...
    @classmethod
    def generate_amortization_schedule(cls, principal: Decimal, annual_rate: Decimal,
                                      term_months: int, start_date: datetime.date,
                                      payment_frequency: str = 'MONTHLY') -> List[Dict]:
        """
    ...
    @classmethod
    def calculate_early_repayment_savings(cls, loan: Loan, repayment_amount: Decimal,
                                         repayment_date: datetime.date) -> Dict:
        """
        Calculate savings from early repayment.
        
        Args:
            loan: Loan object
            repayment_amount: Early repayment amount
            repayment_date: Date of early repayment
...
class CreditScoringService:
    """
    Professional credit scoring service for loan applications.
    Uses multiple factors to calculate credit score and risk rating.
    """
    
    @classmethod
    def calculate_credit_score(cls, client, loan_amount: Decimal, 
                              existing_loans: List[Loan] = None) -> Decimal:
        """
        Calculate credit score (0-100) for a client.
        
        Scoring factors:
        1. Account History (20 points)
        2. Savings Behavior (25 points)
        3. Transaction Patterns (20 points)
        4. Debt Capacity (25 points)
        5. Collateral Quality (10 points)
        """
        score = Decimal('0')
        ...
        # 2. Savings Behavior (25 points)
        # a) Average balance
        avg_balance = client.current_balance  # In real system, calculate average
        ...
        # b) Savings consistency (would require transaction history)
        # For now, assume good consistency
        score += Decimal('8')
        
        # c) Minimum balance maintenance
        ...
        # 3. Transaction Patterns (20 points)
        # In real system, analyze transaction frequency, amounts, patterns
        # For now, use basic scoring
        transaction_score = Decimal('15')  # Base assumption
        ...
        if existing_loans:
            ...
            if hasattr(client, 'monthly_income') and client.monthly_income:
                ...
                # No income data, use savings-based approach
            ...
            # No existing loans - good sign
            score += Decimal('25')
        
        # 5. Collateral Quality (10 points)
        # This would come from loan application
        # For now, assume average collateral
    ...
    def calculate_max_loan_amount(cls, client, existing_loans: List[Loan] = None) -> Decimal:
        """
        Calculate maximum loan amount client can qualify for.
        
        Based on:
        1. Savings multiplier (typically 3-5x savings)
        2. Debt-to-income ratio (max 60%)
        3. Existing debt obligations
        """
        # Base on savings
        savings_multiplier = Decimal('3')  # Conservative multiplier
        max_from_savings = client.current_balance * savings_multiplier
        
        # Consider income if available
        max_from_income = Decimal('0')
        if hasattr(client, 'monthly_income') and client.monthly_income:
            # Max annual debt service = 60% of annual income
            max_annual_debt = client.monthly_income * Decimal('12') * Decimal('0.6')
            
            # Subtract existing debt payments
            if existing_loans:
                existing_annual_payments = sum(
                    loan.total_repayment_amount / Decimal(str(loan.term_days)) * Decimal('365')
                    for loan in existing_loans if loan.status in ['ACTIVE', 'OVERDUE']
                )
                max_from_income = max_annual_debt - existing_annual_payments
            else:
                max_from_income = max_annual_debt
        
        # Take the lower of the two constraints
...
class PaymentProcessingService:
    """
    Service for processing loan payments with intelligent allocation.
    """
    
    @classmethod
    @transaction.atomic
    def process_payment(cls, loan: Loan, amount: Decimal, payment_date: datetime.date,
                       payment_method: str, received_by, notes: str = '',
                       allocation_strategy: str = 'AUTO') -> Dict:
        """
        Process a loan payment with intelligent allocation.
        
        Allocation strategies:
        - AUTO: Automatic allocation (late fees > overdue > current)
        - PRINCIPAL_FIRST: Apply to principal first
        - INTEREST_FIRST: Apply to interest first
        - LATE_FEES_FIRST: Apply to late fees first
        ...
        # Get overdue installments (sorted by due date)
        overdue_installments = loan.repayment_schedule.filter(
            Q(status='OVERDUE') | 
            Q(status='DUE', due_date__lt=payment_date)
        ).order_by('due_date')
        
        # Get current/pending installments
        current_installments = loan.repayment_schedule.filter(
            status__in=['PENDING', 'DUE'],
            due_date__gte=payment_date
        ).order_by('due_date')
        ...
        # STRATEGY 1: LATE FEES FIRST (if applicable)
        if allocation_strategy in ['AUTO', 'LATE_FEES_FIRST']:
            ...
            if total_late_fees > 0 and remaining_amount > 0:
                ...
                # Allocate late fee payment proportionally
                for installment in overdue_installments:
                    ...
                    if installment_payment > 0:
                        ...
                        transaction = LoanTransaction.objects.create(
                            ...
                            amount=installment_payment,
                            fee_amount=installment_payment,
                            transaction_date=payment_date,
                            notes=f"Late fee payment: {notes}",
        ...
        # STRATEGY 2: PAY OVERDUE INSTALLMENTS
        if allocation_strategy in ['AUTO', 'PRINCIPAL_FIRST', 'INTEREST_FIRST']:
            for installment in overdue_installments:
                ...
                # Determine how much to pay this installment
                payment_for_installment = min(installment_balance, remaining_amount)
                
                # Allocate between principal and interest based on strategy
                if allocation_strategy == 'PRINCIPAL_FIRST':
                    principal_payment = min(payment_for_installment, 
                                          installment.principal_amount - installment.paid_principal)
                    interest_payment = payment_for_installment - principal_payment
                elif allocation_strategy == 'INTEREST_FIRST':
                    interest_payment = min(payment_for_installment, 
                                         installment.interest_amount - installment.paid_interest)
                    principal_payment = payment_for_installment - interest_payment
                else:  # AUTO - proportional allocation
                ...
                if principal_payment > 0:
                    transaction = LoanTransaction.objects.create(
                        ...
                        transaction_date=payment_date,
                        notes=f"Principal payment: {notes}",
                        recorded_by=received_by,
                    )
                ...
                if interest_payment > 0:
                    transaction = LoanTransaction.objects.create(
                        ...
                        transaction_date=payment_date,
                        notes=f"Interest payment: {notes}",
                        recorded_by=received_by,
                    )
        ...
        # STRATEGY 3: PAY CURRENT INSTALLMENTS
        if remaining_amount > 0 and allocation_strategy in ['AUTO', 'PRINCIPAL_FIRST', 'INTEREST_FIRST']:
            for installment in current_installments:
                ...
                # Similar allocation logic as above
                if allocation_strategy == 'PRINCIPAL_FIRST':
                    principal_payment = min(payment_for_installment, 
                                          installment.principal_amount - installment.paid_principal)
                    interest_payment = payment_for_installment - principal_payment
        ...
        # If any amount remains after paying all installments, apply as advance payment
        if remaining_amount > 0:
            # Apply to next due installment or as general principal payment
            ...
            if next_installment:
                ...
                # No future installments, apply as general principal reduction
                transaction = LoanTransaction.objects.create(
                    loan=loan,
                    transaction_type='PRINCIPAL_PAYMENT',
                    payment_method=payment_method,
                    ...
                    principal_amount=remaining_amount,
                    transaction_date=payment_date,
                    notes=f"Principal reduction: {notes}",
                    recorded_by=received_by,
                )
                allocation['principal'] += remaining_amount
                allocation['transactions'].append(transaction)
    ...
    def update_loan_balances(cls, loan: Loan):
        ...
        loan.overdue_amount = sum(
            installment.total_amount - installment.total_paid 
            for installment in overdue_installments
        )
        ...
        next_due_installment = loan.repayment_schedule.filter(
            status__in=['PENDING', 'DUE']
        ).order_by('due_date').first()
...
class LoanDisbursementService:
    """
    Service for handling loan disbursement process.
    """
    
    @classmethod
    @transaction.atomic
    def disburse_loan(cls, loan: Loan, disbursement_date: datetime.date,
                     disbursed_by, payment_method: str, transaction_reference: str = '',
                     notes: str = '') -> LoanTransaction:
        """
        Disburse a loan and create all necessary records.
        
        Args:
            loan: Loan to disburse
            disbursement_date: Date of disbursement
            disbursed_by: User disbursing the loan
            payment_method: Method of disbursement
        ...
        disbursement = LoanTransaction.objects.create(
            loan=loan,
        ...
        # Update client account balance (if integrated)
    ...
    def update_client_account(cls, client, amount: Decimal, disbursement_date: datetime.date):
        """
        Update client account after disbursement.
        This should integrate with your client_accounts app.
        """
        try:
            # Add loan amount to client's account balance
            client.current_balance += amount
            
            # Create transaction record in client_accounts
            # This depends on your client_accounts models
            # Example:
            # from client_accounts.models import AccountTransaction
            ...
            # Log error but don't fail disbursement
            import logging
...
class AmortizationService:
    """
    Service for generating and managing amortization schedules.
    """
    
    @classmethod
    def create_repayment_schedule(cls, loan: Loan):
        """
        Create repayment schedule for a loan based on loan type.
        """
    ...
    def create_flat_interest_schedule(cls, loan: Loan) -> List[LoanRepaymentSchedule]:
        """
        Create schedule for flat interest loans.
        """
        # Calculate total interest
        interest_service = InterestCalculationService()
        total_interest = interest_service.calculate_flat_interest(
        ...
        # Determine number of installments based on term
        ...
        for i in range(1, installments + 1):
            ...
            schedule_entry = LoanRepaymentSchedule.objects.create(
                loan=loan,
        ...
        # Update loan's first payment date
        if schedule_entries:
    ...
    def create_reducing_balance_schedule(cls, loan: Loan) -> List[LoanRepaymentSchedule]:
        """
        Create schedule for reducing balance loans (amortized).
        """
    ...
    @classmethod
    def reschedule_loan(cls, loan: Loan, new_term_days: int, 
                       new_interest_rate: Decimal = None, 
                       reschedule_date: datetime.date = None) -> Dict:
        """
        Reschedule a loan with new terms.
        
        Returns:
            Dictionary with old and new schedule details
        """
        ...
        # Use existing interest rate if not specified
        ...
        loan.repayment_schedule.filter(
            due_date__gt=reschedule_date
        ).delete()
...
class LateFeeService:
    ...
    def calculate_late_fees(cls, loan: Loan, as_of_date: datetime.date = None) -> Decimal:
        """
        Calculate late fees for a loan as of specific date.
        
        Args:
            loan: Loan to calculate fees for
        ...
        for installment in overdue_installments:
            ...
            if as_of_date > grace_end_date:
                # Calculate days late beyond grace period
    ...
    def apply_late_fees(cls, loan: Loan, as_of_date: datetime.date = None) -> List[LoanTransaction]:
        """
        Apply late fees to a loan and create fee transactions.
        
        Returns:
            List of created fee transactions
        """
        if not as_of_date:
            as_of_date = timezone.now().date()
            ...
            if as_of_date > grace_end_date:
                ...
                if overdue_amount > Decimal('0'):
                    ...
                    transaction = LoanTransaction.objects.create(
                        ...
                        notes=f'Late fee for installment #{installment.installment_number}',
...
class ReportService:
    """
    Service for generating various loan reports.
    """
    
    @classmethod
    def generate_portfolio_report(cls, start_date: datetime.date = None,
        ...
        return {
            ...
            'summary': {
                ...
                'average_loan_size': total_principal / total_loans if total_loans > 0 else Decimal('0'),
            },
    ...
    @classmethod
    def generate_repayment_collection_report(cls, start_date: datetime.date,
        ...
        return {
            ...
            'summary': {
                ...
                'average_collection': total_collected / (end_date - start_date).days if (end_date - start_date).days > 0 else Decimal('0'),
            },
            'by_method': by_method,
            'by_day': by_day,
    ...
    def generate_overdue_loans_report(cls, as_of_date: datetime.date = None) -> Dict:
        ...
        overdue_loans = Loan.objects.filter(
            status='OVERDUE'
        ).select_related('client', 'loan_product')
        ...
        for loan in overdue_loans:
            ...
            report_data.append({
                ...
                'client_name': loan.client.full_account_name,
                'client_phone': loan.client.phone_number,
                ...
                'late_fees': late_fees,
                'loan_officer': f"{loan.loan_officer.first_name} {loan.loan_officer.last_name}",
            })
        ...
        return {
            ...
            'total_overdue_loans': len(report_data),
...
class NotificationService:
    """
    Service for sending notifications (SMS, Email, In-app).
    """
    
    @classmethod
    def send_payment_reminder(cls, installment: LoanRepaymentSchedule):
        """
        Send payment reminder for an upcoming installment.
        """
        loan = installment.loan
        client = loan.client
        
        # Check if reminder already sent today
        reminder_sent_today = False  # You'd check from a notification log
        
        if not reminder_sent_today:
            # Send SMS
            sms_message = (
                f"Dear {client.full_account_name}, "
                f"Your loan payment of {installment.total_amount} "
                f"for loan {loan.loan_number} is due on {installment.due_date}. "
    ...
    def send_late_payment_notification(cls, installment: LoanRepaymentSchedule, days_late: int):
        ...
        sms_message = (
            f"Dear {client.full_account_name}, "
            f"Your loan payment of {installment.total_amount} "
            f"for loan {loan.loan_number} is {days_late} days overdue. "
            f"Late fee: {late_fee}. Please make immediate payment."
        )
        cls.send_sms(client.phone_number, sms_message)
        ...
        if hasattr(loan_officer, 'phone_number') and loan_officer.phone_number:
            officer_message = (
                f"Loan {loan.loan_number} for {client.full_account_name} "
                f"is {days_late} days overdue. Amount: {installment.total_amount}"
            )
...
def is_valid_phone_number(phone: str) -> bool:
    ...
    import re
    # Ugandan phone number pattern
    pattern = r'^(\+256|0)[7|8][0-9]{8}$'
