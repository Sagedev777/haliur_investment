...
def export_financial_csv(request):
    ...
    writer = csv.writer(response)
    writer.writerow(['Report Type', 'Date', 'Total Accounts', 'Active Accounts', 'Total Savings', 'Loans Disbursed', 'Pending', 'Approved', 'Completed', 'Defaulted', 'Interest Earned', 'Guarantors', 'Transactions'])
    for report in reports:
        writer.writerow([report.get_report_type_display(), report.report_date, report.total_accounts, report.active_accounts, report.total_savings, report.total_loans_disbursed, report.total_loans_pending, report.total_loans_approved, report.total_loans_completed, report.total_loans_defaulted, report.total_interest_earned, getattr(report, 'total_guarantors', 0), getattr(report, 'total_transactions', 0)])
    return response
...
def export_financial_pdf(request):
    ...
    y = 520
    headers = ['Report Type', 'Date', 'Total Accounts', 'Total Savings', 'Loans Disbursed', 'Interest Earned', 'Guarantors', 'Transactions']
    ...
    for report in reports:
        if y <= 120:
            p.showPage()
            y = 550
        data = [report.get_report_type_display(), report.report_date.strftime('%b %d, %Y'), str(report.total_accounts), f'{report.total_savings:,.2f}', f'{report.total_loans_disbursed:,.2f}', f'{report.total_interest_earned:,.2f}', str(getattr(report, 'total_guarantors', 0)), str(getattr(report, 'total_transactions', 0))]
...
def export_summary_pdf(request):
    ...
    if reports.exists():
        ...
        y = 750
        p.setFont('Helvetica', 12)
        summary = [('Report Date:', latest.report_date.strftime('%B %d, %Y')), ('Total Accounts:', latest.total_accounts), ('Active Accounts:', latest.active_accounts), ('Total Savings (UGX):', f'{latest.total_savings:,.2f}'), ('Total Loans Disbursed (UGX):', f'{latest.total_loans_disbursed:,.2f}'), ('Interest Earned (UGX):', f'{latest.total_interest_earned:,.2f}'), ('Defaulted Loans:', latest.total_loans_defaulted)]
