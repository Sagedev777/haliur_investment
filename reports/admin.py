...
from loans.models import LoanApplication, LoanTransaction
...
class SystemReportAdmin(admin.ModelAdmin):
    list_display = ['report_type', 'report_date', 'total_accounts', 'total_savings', 'total_loans_disbursed', 'total_loans_pending', 'total_loans_approved', 'total_loans_completed', 'total_loans_defaulted', 'total_interest_earned', 'generated_by', 'generated_at']
    ...
    def report_details(self, obj):
        details = [f'📊 {obj.get_report_type_display()} Report for {obj.report_date}', f'👥 Accounts: {obj.total_accounts} total, {obj.active_accounts} active', f'💰 Savings: {obj.total_savings}', f'🏠 Loans: {obj.total_loans_disbursed} disbursed', f'📋 Status: Pending {obj.total_loans_pending}, Approved {obj.total_loans_approved}', f'✅ Completed {obj.total_loans_completed}, ❌ Defaulted {obj.total_loans_defaulted}', f'💵 Interest Earned: {obj.total_interest_earned}']
        return '\n'.join(details)
    ...
    def export_to_csv(self, request, queryset):
        ...
        writer.writerow(['Report Type', 'Date', 'Total Accounts', 'Active Accounts', 'Total Savings', 'Total Loans Disbursed', 'Pending Loans', 'Approved Loans', 'Completed Loans', 'Defaulted Loans', 'Interest Earned'])
        for report in queryset:
            writer.writerow([report.get_report_type_display(), report.report_date, report.total_accounts, report.active_accounts, report.total_savings, report.total_loans_disbursed, report.total_loans_pending, report.total_loans_approved, report.total_loans_completed, report.total_loans_defaulted, report.total_interest_earned])
        return response
